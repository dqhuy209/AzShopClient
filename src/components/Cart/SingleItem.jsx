...
const SingleItem = ({ item }) => {
  ...
  return (
    <div className="grid items-center grid-cols-12 gap-4 border-t border-gray-3 py-5 px-7.5">
      ...
      <div className="col-span-3">
        <p className="text-sm text-blue sm:text-base">{formatVNDRounded.thousands((item.discountedPrice || 0) * (item.quantity || 1))}</p>
      </div>
      ...
      <div className="flex justify-end col-span-3">
        <button
          ...
          className="flex md:mr-5 items-center justify-center rounded-lg max-w-[32px] h-8 sm:max-w-[38px] sm:h-9.5 w-full bg-gray-2 border border-gray-3 text-dark ease-out duration-200 hover:bg-red-light-6 hover:border-red-light-4 hover:text-red"
        >
          <svg
            className="fill-current w-4 h-4 sm:w-[22px] sm:h-[22px]"
            width="22"
            height="22"
            viewBox="0 0 22 22"
            fill="none"
