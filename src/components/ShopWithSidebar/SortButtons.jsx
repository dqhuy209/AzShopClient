...
const SortButtons = ({ productSidebar, setProductSidebar }) => {
  ...
  const Button = ({ active, onClick, children }) => (
    <button
      type="button"
      onClick={onClick}
      className={` px-[4px] py-[6px] lg:px-2 lg:py-3 border text-sm transition-colors flex items-center gap-x-[10px] group ${
        active
          ? 'text-blue border-blue font-semibold bg-white '
          : ' text-dark border-gray-200 hover:border-blue hover:text-blue bg-[#f9f9f9] hover:bg-white'
      }`}
    >
      {children}
    </button>
      ...
      <div className="flex flex-wrap items-center gap-2.5">
          ...
          <div className="w-[20px] h-[20px]">
            ...
            >
              <path
                ...
                strokeWidth="1.5"
                strokeLinecap="round"
                className={`stroke-[#000] group-hover:stroke-[#dd2439] transition-colors ${isAllActive ? 'stroke-[#dd2439]' : ''}`}
              />
              <path
          ...
          <div className="w-[20px] h-[20px]">
            ...
            >
              <path
                ...
                strokeWidth="1.5"
                strokeLinecap="round"
                className={`stroke-[#000] group-hover:stroke-[#dd2439] transition-colors transition-colors' ${isFeaturedActive ? 'stroke-[#dd2439]' : ''}`}
              />
            </svg>
          </div>
          Bán chạy nhất
        ...
        <Button active={isLatestActive} onClick={handleLatest}>
          <div className="w-[20px] h-[20px]">
            ...
            >
              <path
                className={`stroke-[#000] group-hover:stroke-[#dd2439] transition-colors ${isLatestActive ? 'stroke-[#dd2439]' : ''}`}
                fill="currentColor"
        ...
        <Button active={isPriceAsc} onClick={handlePriceAsc}>
          <div className="w-[20px] h-[20px]">
            ...
            >
              <path
                clipRule="evenodd"
                className={`stroke-[#000] group-hover:stroke-[#dd2439] transition-colors ${isPriceAsc ? 'stroke-[#dd2439]' : ''}`}
        ...
        <Button active={isPriceDesc} onClick={handlePriceDesc}>
          <div className="w-[20px] h-[20px]">
            ...
            >
              <path
                className={`stroke-[#000] group-hover:stroke-[#dd2439] transition-colors ${isPriceDesc ? 'stroke-[#dd2439]' : ''}`}
                fillRule="evenodd"
                clipRule="evenodd"
